...
public sealed class T100_ClioTests
{
	...
	{
		return new TestingOptions( s => Assert( false ), "TestApp", screenWidth: null );
	}
	...
	{
		ArgumentParser argumentParser = new( verbTerm, getTestingOptions() );
	...
	public void T213_Switch_May_Not_Be_Added_After_Verb()
	{
		...
		Assert( exception.ViolatingArgumentName == "bravo" );
		Assert( exception.PrecedingArgumentName == verbTerm );
	}
